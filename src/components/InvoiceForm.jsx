...
function InvoiceForm() {
  ...
  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('next_invoice_no') || 'null');
      if (saved && typeof saved === 'object') {
        const { prefix, width, nextNum } = saved;
        setInvoiceInfo((prev) => ({ ...prev, number: `${prefix}${String(nextNum).padStart(width, '0')}` }));
        return;
      }
    } catch {}
    const m = String(invoiceInfo.number || 'INV00001').match(/^([^\d]*)(\d+)$/);
    const prefix = m ? m[1] : 'INV';
    const digits = m ? m[2] : '00001';
    const width = digits.length;
    const nextNum = parseInt(digits, 10);
    localStorage.setItem('next_invoice_no', JSON.stringify({ prefix, width, nextNum }));
    setInvoiceInfo((prev) => ({ ...prev, number: `${prefix}${String(nextNum).padStart(width, '0')}` }));
  }, []);
  ...
  const handleSaveInvoice = () => {
    ...
    const m = String(invoiceInfo.number || "").match(/^([^\d]*)(\d+)$/);
    const prefix = m ? m[1] : "INV";
    const digits = m ? m[2] : "00001";
    const width = digits.length;
  ...
  const taxAmount = tax.type.toLowerCase() === "none"
    ...
    : items.reduce((acc, item) => {
        ...
        if (tax.inclusive) {
          const taxPart = itemTotal - (itemTotal / (1 + TAX_RATE));
          return acc + taxPart;
        } else {
          return acc + itemTotal * TAX_RATE;
        }
      }, 0);
                  ...
                  <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
                    ...
                    <Box mt={3} sx={{ textAlign: "right", pr: 2 }}>
                      ...
                      <Typography variant="subtitle1" fontWeight="bold" sx={{ mt: 0.5 }}>
                        Balance Due: {currencySymbol}{total.toFixed(2)}
                      </Typography>
                    </Box>
